...
class ADRGenerator:
    ...
    def generate(
        ...
        # Agent Context Brief — machine-readable block for system prompt injection
        # AI agents reading this ADR file can extract this section without calling
        # the MCP server again, saving tokens on every future interaction.
        if recommendation.context_brief:
            lines += [
                "---",
                "",
                "## Agent Context Brief",
                "",
                "> Copy the block below directly into your AI agent's system prompt.",
                "> It is a compressed (<200 token) architecture brief validated by TrueArch.",
                "",
                "```",
                recommendation.context_brief,
                "```",
                "",
            ]
